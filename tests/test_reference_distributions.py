...
class TestGenerateReferenceDistributions:
    ...
    def test_decile_boundaries_have_11_values(self, tmp_path: Path, training_features) -> None:
        from demandops.training.reference_distributions import (
            generate_reference_distributions,
        )
    ...
    def test_ks_subsample_capped(self, tmp_path: Path, training_features) -> None:
        ...
        output_path = tmp_path / "ref.json"
        generate_reference_distributions(training_features, output_path, ks_subsample_size=500)
        ref = json.loads(output_path.read_text())
        for feature_name in FEATURE_COLUMNS:
    ...
    def test_metadata_records_subsample_size(self, tmp_path: Path, training_features) -> None:
        from demandops.training.reference_distributions import (
            generate_reference_distributions,
        )
    ...
    def test_constant_column_produces_no_nan(self, tmp_path: Path, training_features) -> None:
        from demandops.training.reference_distributions import (
            generate_reference_distributions,
        )
